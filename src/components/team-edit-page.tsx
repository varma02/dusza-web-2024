...
import { Button, Card, CardBody, CardHeader, Divider, User, Chip, CardFooter, Modal, ModalContent, useDisclosure, ModalHeader, ModalBody, ModalFooter, Input, Select, SelectItem } from "@nextui-org/react"
...
export const TeamEditPage = ({
  ...
  const category = categories.find(category => category.id === team?.category_id)
  const programmingLanguage = programmingLanguages.find(language => language.id === team?.programming_language_id)

  const fixMembers = teamMembers.filter(member => !member.substitute)
  const subMember = teamMembers.find(member => member.substitute)

  const { register, handleSubmit, formState: { isSubmitting, errors }, reset } = useForm<z.infer<typeof teamEditSchema>>({
  ...
  const onSubmit = async (values: z.infer<typeof teamEditSchema>) => {
    const res = await handleTeamEdit({ id: team?.id, teamMembers: [ ...fixMembers, subMember ], values })
    ...
    <main className="grid place-items-center flex-1 p-4">
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>Csapat szerkesztése</ModalHeader>
              <form
      ...
      <Card className="w-full max-w-xl p-2">
        <CardHeader className="flex justify-between items-center gap-4">
          <h2 className="w-full text-xl text-center font-semibold">{team?.name} csapat adatai</h2>
        </CardHeader>
        <CardBody className="flex flex-wrap gap-8">
          <div className="min-w-max flex-1">
            <div className="flex items-center gap-4 pb-2">
              <span className="text-sm whitespace-nowrap text-foreground-500">Csapattagok</span>
              <Divider className="flex-1" />
            </div>
            <li className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {teamMembers.map(member => (
                <ul key={member.id}>
                  <User
                    name={member.substitute ? `${member.name} | Pót` : member.name}
          ...
          <div className="min-w-[minmax(max,100%)] flex-1">
            <div className="flex items-center gap-4 pb-2">
              <span className="text-sm whitespace-nowrap text-foreground-500">Felkészítők</span>
              <Divider className="flex-1" />
            </div>
          ...
          </div>
          {
            ...
            <div className="min-w-[minmax(max,100%)] flex-1">
              <div className="flex items-center gap-4 pb-2">
                <span className="text-sm whitespace-nowrap text-foreground-500">Kategória</span>
                <Divider className="flex-1" />
              </div>
              <li className="flex flex-col gap-4 md:gap-0">
                ...
                <ul className="flex flex-col md:flex-row justify-between gap-0 md:gap-4">
                  ...
                  <i>{dateFormatter.format(new Date(category.valid_from))}</i>
                </ul>
                <ul className="flex flex-col md:flex-row justify-between gap-0 md:gap-4">
                  <span>Jelentkezés vége:</span>
                  <i>{dateFormatter.format(new Date(category.valid_until))}</i>
                </ul>
              </li>
            </div>
            ) : null
