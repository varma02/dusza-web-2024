        ...
        { teams ? (
            teams.map(team => (
              <Button key={team.id} className="text-xl h-auto p-4 rounded-r-none justify-start pr-10" variant={currentTeam?.id === team.id ? "solid" : "light"} onClick={() => setCurrentTeam(team)}>
                ...
                <span className="text-wrap text-left">{team.name}</span>
              </Button>
            ))
              ...
              <ul className="flex flex-col">
                {
                  ...
                  messages.map(message => (
                    ...
                    <li key={message.id} className={`w-full flex flex-col p-2 gap-2 ${message.author_id === user?.id && "items-end"}`}>
                      ...
                      <span className={`text-lg bg-content2 p-2 rounded-lg text-wrap whitespace-pre-line ${message.author_id === user?.id && "bg-primary-100"} max-w-[70%] w-max`}>
                        {message.message}
                      </span>
                    </li>
                  ))
